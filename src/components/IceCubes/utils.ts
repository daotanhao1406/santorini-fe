...
export function generateColumnElements(
  ...
  for (let i = 0; i < cubeCount; i++) {
    ...
    const allowBigSize =
      (i === firstBigCubeIndex || i === secondBigCubeIndex) && cubeCount >= 4
...
export function processElementsForBlur(
  ...
  const lightBlurThreshold =
    sortedBySize[Math.floor(totalCount * 0.8)]?.size || 0 // Next 20% - light blur
